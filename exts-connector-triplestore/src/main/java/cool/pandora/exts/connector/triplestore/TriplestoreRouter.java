...
package cool.pandora.exts.connector.triplestore;
...
public class TriplestoreRouter extends RouteBuilder {
    ...
    private static final String RESOURCE_DELETION = "http://fedora"
            + ".info/definitions/v4/event#ResourceDeletion";
    ...
    public void configure() throws Exception {
        ...
        /*
         * Route a message to the proper handler, based on whether it is a DELETE or UPDATE
         * operation.
        ...
        from("direct:delete.triplestore").routeId("AcrepoTriplestoreDeleter")
                .log(LoggingLevel.INFO, LOGGER, "Deleting Triplestore Graph "
        ...
        from("direct:update.triplestore")
                ...
                .log(LoggingLevel.INFO, LOGGER, "Indexing Triplestore Object "
                        + "${headers[CamelFcrepoUri]}")
                .setHeader(HTTP_METHOD).constant("POST")
                .setHeader(CONTENT_TYPE).constant("application/x-www-form-urlencoded; "
                + "charset=utf-8")
